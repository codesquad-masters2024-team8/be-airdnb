...
public class HostroomItemsService {
  ...
  public List<OfferedItemDTO> getOfferedItemsByHostroomId(Long hostroomId) {
    return hostroomItemsRepository.findAll().stream()
        .filter(item -> item.getHostroom().getId().equals(hostroomId))
        .map(item -> new OfferedItemDTO(item.getOfferedItem().getId(), item.getOfferedItem().getItemName()))
        .collect(Collectors.toList());
  }
}
